...
```{r}
library(mtSusie)
mt.susie <- mt_susie(X, Y, L=15, prior.var = 0.01, local.residual = T,
                     hard.selection = T, hard.lodds.cutoff = 2)
```
...
.show <- unique(.dt[alpha > .01]$variants)
.dt <- .dt[variants %in% .show & lfsr < .01]

ggplot(.dt, aes(x=as.factor(variants), y=as.factor(traits))) +
    facet_wrap(.~`levels`, scales="free_x", nrow=1) +
